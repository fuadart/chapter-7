...
exports.userValidationLogin = body("username").custom(async (value, { req }) => {
...
exports.putEditUser = async (req, res, next) => {
  try {
    const userId = req.params.id;
    const { username, email, password, firstName, lastName, nationality, tribe } = req.body;
    const encryptedPassword = bcrypt.hashSync(password, 10);

    await User.update({ username, email, password: encryptedPassword }, { where: { id: userId } });
    await User_biodata.update({ firstName, lastName, nationality, tribe }, { where: { userId } });
