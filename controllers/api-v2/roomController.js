...
const calcResult = (player1Pick, player2Pick) => {
  ...
  if (player1Pick === "R") return player2Pick === "S" ? "PLAYER 1 WIN" : "PLAYER 2 WIN";
  if (player1Pick === "P") return player2Pick === "R" ? "PLAYER 1 WIN" : "PLAYER 2 WIN";
  if (player1Pick === "S") return player2Pick === "P" ? "PLAYER 1 WIN" : "PLAYER 2 WIN";
};
...
exports.playerJoin = async (req, res, next) => {
  try {
    if (!req.body.id || !req.body.player2Id) throw new Error("Please input room id and player 2 id");

    const roomId = req.body.id;
    const matchRoom = await Room.findOne({ where: { id: roomId } });
    if (!matchRoom) throw new Error(`Could not find room with id : ${roomId}`);
    if (matchRoom.player2Id !== null) throw new Error(`Another player already joined this room`);

    const roomArray = await Room.update({ player2Id: req.body.player2Id }, { where: { id: roomId }, returning: true });
...
exports.playGame = async (req, res, next) => {
  ...
  if (player === "Player 1") {
    ...
    if (!req.body.pick) throw new Error(`Please pick your choices between "R", "P", "S"`);

    player1Choices.push(req.body.pick);
    const roomUpdate = await Room.update({ player1Choices }, { where: { id: req.params.room_id }, returning: true });
    const [_, room] = [...roomUpdate];

    const result = getResult(room[0].player1Choices, room[0].player2Choices, matchInfo);

    await Room.update({ matchInfo: result }, { where: { id: req.params.room_id } });
    ...
    const roomUpdate = await Room.update({ player2Choices }, { where: { id: req.params.room_id }, returning: true });
    const [_, room] = [...roomUpdate];
